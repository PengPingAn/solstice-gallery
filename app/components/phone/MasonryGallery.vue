...
// 从日期字符串中提取年份和月份
const getYearMonthFromDate = (dateStr: string): { year: number; month: number } | null => {
...
const addressText = computed(() => {
  ...
  const addresses = Array.from(visibleAddresses.value).filter((addr) => addr && addr.trim() !== '')

  const maxDisplayAddresses = 3
  if (addresses.length > maxDisplayAddresses) {
    return `${addresses.slice(0, maxDisplayAddresses).join('、')} 等 ${addresses.length} 个地点`
  }
