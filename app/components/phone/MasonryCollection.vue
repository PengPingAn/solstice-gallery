        ...
        <template v-for="(col, colIndex) in deviceColumns" :key="colIndex">
          <div class="flex-1 flex flex-col">
            ...
            >
              ...
              />
              <!-- 存储来源标签（仅显示阿里云，避免遮挡选择按钮） -->
              <div
                v-if="item.storageType === 'aliyun-oss'"
                class="absolute bottom-2 left-2 z-10 pointer-events-none"
              >
                <UBadge color="neutral" variant="soft" size="sm">阿里云</UBadge>
              </div>
            </div>
          </div>
        </template>
