...
const props = defineProps<{
  ...
  resolution?: string
  size?: string
  renderType?: 'canvas' | 'img' // 新增：渲染类型，默认canvas
}>()

const canvasRef = ref<HTMLCanvasElement | null>(null)
const imgRef = ref<HTMLImageElement | null>(null) // 新增：img引用
...
const updateVisibility = () => {
  ...
  // 根据渲染类型选择不同的元素
  const displayElement = props.renderType === 'img' ? imgRef.value : canvasRef.value
  ...
  // 使用元素的实际高度或容器的较小值
  const actualHeight = elementRect.height || containerRect.height
...
const loadImgImage = () => {
  ...
  imgRef.value.crossOrigin = 'Anonymous'
...
<template>
  ...
  >
    ...
    <img
      ...
      ref="imgRef"
      :src="thumbSrc || src"
      :srcset="thumbSrc ? `${thumbSrc} 480w, ${src} 1600w` : undefined"
    ...
    <div class="pointer-events-none cursor-pointer">
      ...
      <div class="absolute inset-x-0 bottom-0 p-4 text-white">
        <div class="**:duration-300 space-y-2 min-h-[56px]">
          ...
          <div v-if="props.meta" class="flex flex-wrap gap-1.5">
            <Tag
              size="sm"
              radius="full"
        ...
        <!-- <div
          ...
          class="flex items-center gap-2 pt-2 text-xs opacity-0 group-hover:opacity-100"
        > -->
        <div
          v-if="props.address || props.date"
          class="flex items-center gap-2 pt-2 text-xs opacity-0 group-hover:opacity-100"
        >
          <Tag
            v-if="props.address"
